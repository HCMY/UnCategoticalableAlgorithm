...
class HongZha:
    ...
    def meituan(self,name):
        try:
            driver.implicitly_wait(10)
            driver.get('http://passport.meituan.com/account/unitivelogin?service=movie&continue=http%3A%2F%2Fugc.movie.meituan.com%2Fauth&_nsmobilelogin=true')
            tel = driver.find_element_by_xpath('//*[@id="login-mobile"]')
            tel.click()
            tel.send_keys(self.phone)
            button = driver.find_element_by_xpath('//*[@id="J-mobile-form"]/div[4]/div')
            self.send_yzm(button, name)
        except:
            print('本次失败')
    ...
    def shangfatong(self, name):
        try:
            driver.get('https://www.blb.com.cn/aclogin.html')
            tel = driver.find_element_by_xpath('//*[@id="telephone"]')
            tel.send_keys(self.phone)
            btn = driver.find_element_by_xpath('//*[@id="num"]')
            self.send_yzm(btn, name)
        except:
            print('本次失败')
    def vivo(self, name):
        try:
            driver.implicitly_wait(10)
            driver.get('https://passport.vivo.com.cn/#/register')
            tel = driver.find_element_by_xpath('/html/body/div[1]/div[3]/div[1]/div[3]/div/div[3]/input')
            tel.send_keys(self.phone)
            driver.find_element_by_xpath('/html/body/div[1]/div[3]/div[1]/div[3]/div/button').click()
            driver.get('https://passport.vivo.com.cn/#/registerSetPwd')
            btn = driver.find_element_by_xpath('/html/body/div[1]/div[3]/div[1]/div[3]/div/div[2]/div')
            self.send_yzm(btn,name)
        except:
            print('本次失败')
    ...
    def baidu(self,name):
        try:
            ...
            driver.find_element_by_xpath('//*[@id="TANGRAM__PSP_3__smsSwitchWrapper"]').click()
            tel = driver.find_element_by_xpath('//*[@id="TANGRAM__PSP_3__smsPhone"]')
            tel.send_keys(self.phone)
            btn = driver.find_element_by_xpath('//*[@id="TANGRAM__PSP_3__smsTimer"]')
            self.send_yzm(btn, name)
        except:
            print('本次失败')

    def china_mobile(self,name):
        try:
            driver.implicitly_wait(10)
            driver.get('https://login.10086.cn/login.html?channelID=12034&backUrl=http%3A%2F%2Fwww.10086.cn%2Findex%2Fbj%2Findex_100_100.html%3FWT.mc_id%3D8t0xsDMfVa8hgIN138jM_HSOWBEsWdp9csP4WmnHn1550300269.553wm0x175d154o189t100xm0w')
            driver.find_element_by_xpath('//*[@id="sms_login_1"]').click()
            tel = driver.find_element_by_xpath('//*[@id="sms_name"]')
            tel.send_keys(self.phone)
            btn = driver.find_element_by_xpath('//*[@id="getSMSPwd1"]')
            self.send_yzm(btn, name)
        except:
            print('本次失败')
